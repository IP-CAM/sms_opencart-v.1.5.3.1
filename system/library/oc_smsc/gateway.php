...
class OCSMSCGateway {
	...
	public function get_order_info($textarea, $order_id, $order_status_id = 0) {
		...
		$macros = array('~\{ORDER_NUM\}~i', '~\{ORDER_SUM\}~i', '~\{ORDER_STATUS\}~i', '~\{FIRST_NAME\}~i', '~\{LAST_NAME\}~i', '~\{CR_PHONE\}~i', '~\{CR_CITY\}~i', '~\{CR_ADDR\}~i', '~\{GOODS_LIST\}~i', '~\{COMMENT\}~i');
		...
		$value = str_replace('$', '\$', array($order_id, $total, $status->row['name'],
												...
												$order->row['payment_address_1'].' '.$order->row['payment_address_2'], $g_list, $order->row['comment']));
